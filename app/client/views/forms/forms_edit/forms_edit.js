Template.FormsEdit.onCreated(function() {
...
Template.FormsEdit.events({
  'submit form': function(e) {
    ...
    Meteor.call('formsEdit', form, function (error, result) {
      if (error) {
        console.log(error);
      ...
      if ($('#phonemic').attr('disabled'))
        ...
        $('#phometic').focus();
      return Materialize.toast("Changed form '" + (form.phonemic || form.phonetic) + "'", 4000);
    });
  },
...
Template.FormsEdit.rendered = function() {
  ...
  $('.datepicker').val(Forms.findOne({_id: Router.current().params.id}).dateCollected.toString().split(' ').slice(1, 4).join(' '));
